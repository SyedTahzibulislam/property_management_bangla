...
<body id="bodysellcorner">
    ...
    @endif
		...
		<h2>Product Sell Point</h2>
  <span id="form_result"></span>
	
		<form method="post" action="{{ route('producttransition.store') }}"   id="sample_form" class="form-horizontal" enctype="multipart/form-data">
          @csrf
...
<div class="row">
    ...
    <div id="dis2"   class="col-4 ">
	 ...
	 Discount on Total(%):  <input type="text" name="percentofdicountontaotal" id="percentofdicountontaotal"  value="0" autocomplete="off" class="form-control  percentofdicountontaotal"  />
 ...
 Discount :  <input type="text" name="discountatend" id="discountatend" autocomplete="off"  value="0"  class="form-control  discountatend" readonly  />
...
$(document).ready(function(){
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('producttransition.index') }}",
        columns: [
...
refresh();
 ...
 $('.medicine_name').select2();
  ...
  {
   $.ajax({
    url:"{{ route('producttransition.update') }}",
    method:"POST",
    data:new FormData(this),
    contentType: false,
    cache: false,
