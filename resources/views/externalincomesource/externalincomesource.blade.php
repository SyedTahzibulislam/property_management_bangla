...
<body>
  ...
  <span id="form_result"></span>
		...
		<form method="post" action=	"{{ route('externalincomesource.store') }}"   id="sample_form" class="form-horizontal" enctype="multipart/form-data">
          @csrf
			 ...
			 <div class="row">
			 			 <div class="col-4" >
			Income Source:   <input type="text" autocomplete="off"  name="incomename" id="expensesname" class="form-control numbers  expensesname" required  />
  ...
  <div class="modal-content">
   <div class="modal-header">
        ...
        <div class="modal-body">
         <span id="form_result"></span>
         <form method="post" id="sample_form"    action = "{{ route('externalincomesource.store') }}"   class="form-horizontal" enctype="multipart/form-data">
          @csrf
...
$(document).ready(function(){
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('externalincomesource.index') }}",
        columns: [
...
$('#sample_form').on('submit', function(event){
  ...
  {
   $.ajax({
    url:"{{ route('externalincomesource.store') }}",
    method:"POST",
...
fetch();
 ...
 $('.medicine_name').select2();
  ...
  {
   $.ajax({
    url:"{{ route('externalincomesource.update') }}",
    method:"POST",
    data:new FormData(this),
    contentType: false,
    cache: false,
...
var id = $('#parentid').val();
 ...
 $.ajax({
   url:"externalincomesource/dropdownlistforchild/"+id,
   dataType:"json",
 ...
 console.log("AA");	
               ...
               $latest_tr.find(".medicine_name").append(optionformedicine);
					...
					///////////////////////   set dynamic select option values from Database ///////////////////
					                    ...
					                    var len = 0;
                    if (response.incomechild != null) {
                        len = response.incomechild.length;
                    }

                    if (len>0) {
                        for (var i = 0; i<len; i++) {
                             var id = response.incomechild[i].id;
                             var name = response.incomechild[i].name;	 
							 var optionformedicine = "<option     value='"+id+"'>"+name+"</option>"; 
                              $latest_tr.find(".medicine_name").append(optionformedicine);
...
//////////////////////////////////////// code to read selected table row cell data (values) to print invoice
 ...
 $('#ok_button').click(function(){
  $.ajax({
   url:"externalincomesource/destroy/"+user_id,
   beforeSend:function(){
