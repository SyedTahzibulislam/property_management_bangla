...
<div class="container"   style="background-color:#EEE8AA; "  >
          ...
          <form  method="post" id="sample_form" action="{{route('productcaategory.store')}}" class="form-horizontal" enctype="multipart/form-data"  onsubmit="event.preventDefault()">
  ...
  if (  (event.keyCode === 13) || (event.keyCode === 40) ) {
   $check.eq($check.index(this) + 1).focus();
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('productcaategory.index') }}",
...
$('#sample_form').on('submit', function(event){
  ...
  if($('#action').val() == 'Add')
  {
   ...
   $.ajax({
    url:"{{ route('productcaategory.store') }}",
    method:"POST",
  ...
  {
   $.ajax({
    url:"{{ route('productcaategory.update') }}",
    method:"POST",
    data:new FormData(this),
    contentType: false,
    cache: false,
 ...
 $('#ok_button').click(function(){
  $.ajax({
   url:"productcaategory/destroy/"+user_id,
   beforeSend:function(){
