        ...
        columns: [
		 ...
		 {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            ...
            {data: 'name', name: 'name'},
			   ...
			   {data: 'coversionamount', name: 'coversionamount'},
						{data: 'basicunit.name', name: 'basicunit.name'},    
...
fetch();
 ...
 $("#basicunit").html("");
  $.ajax({
   url:"unitconversion/dropdownlist",
   dataType:"json",
...
$('#sample_form').on('submit', function(event){
  ...
  {
   $.ajax({
    url:"{{ route('unitconversion.store') }}",
    method:"POST",
  ...
  {
   $.ajax({
    url:"{{ route('unitconversion.update') }}",
    method:"POST",
    data:new FormData(this),
    contentType: false,
    cache: false,
 ...
 $(document).on('click', '.edit', function(){
  ...
  $('#form_result').html('');
  $.ajax({
   url:"/unitconversion/"+id+"/edit",
   dataType:"json",
   success:function(html){
    $('#conversionunit').val(html.data.coversionamount);
 ...
 $('#ok_button').click(function(){
  $.ajax({
   url:"unitconversion/destroy/"+user_id,
   beforeSend:function(){
