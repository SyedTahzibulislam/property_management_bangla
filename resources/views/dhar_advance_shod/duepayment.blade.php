...
$(document).ready(function(){
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('supplierduepayemnt.index') }}",
        columns: [
	  ...
	  {data: 'comment', name: 'comment'},
               ...
               {data: 'created', name: 'created'}, 
			    ...
			    {data: 'action', name: 'action'}, 
				 ...
				 {data: 'superviser', name: 'accountant'},					
...
function fetch()
{
 ...
 $.ajax({
   url:"{{ route('supplierduepayemnt.dropdownlist') }}",
   dataType:"json",
				   ...
				   var len = 0;
                    ...
                    if (len>0) {
                        for (var i = 0; i<len; i++) {
                            ...
                            var presentduebalance = response.supplier[i].due;
...
$('#sample_form').on('submit', function(event){
  ...
  if($('#action').val() == 'Add')
  {
   ...
   $.ajax({
    url:"{{ route('supplierduepayemnt.store') }}",
    method:"POST",
...
$('#datePicker').val(new Date().toDateInputValue());	
 ...
 $('#ok_button').click(function(){
  $.ajax({
	  ...
	  url:"supplierduepayemnt/destroy/"+user_id,
   beforeSend:function(){
